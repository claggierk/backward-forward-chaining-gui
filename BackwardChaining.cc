	...
	if(rulesFile.is_open())
	{
		...
		while(! rulesFile.eof())
		{
			...
			while(currentWord != "THEN")
			{
				...
				currentClause.push_back(ClauseVariableElement(currentWord, -1));
...
{
	cout << endl << "Please enter one of the following conclusions as you goal:" << endl;
	fout << endl << "Please enter one of the following conclusions as you goal:" << endl;
	...
	conclusionRequest = "Poison";

	ostringstream oss;
	oss << conclusionRequest;
	::gtk_label_set_text(goalText, (const gchar *)( &( oss.str().at(0) ) ) );
	...
	while(conclusionStack.size() > 0)
	{
		...
		else
		{
			...
			if(diagnosisFound)
			{
				...
				PoisonsDiagnosed.push_back(poison);
			}
			else
			{
				string poison = GetSpecificPoison(conclusionIndex);
	...
	if(PoisonsDiagnosed.size() > 0)
	{
		for(unsigned poison = 0; poison < PoisonsDiagnosed.size(); poison++)
		{
			cout << endl << " ****************************************************"
				<< endl << " ***** Diagnosis: " << PoisonsDiagnosed.at(poison) << " Poison";

			fout << endl << " ****************************************************"
				<< endl << " ***** Diagnosis: " << PoisonsDiagnosed.at(poison) << " Poison";

			ostringstream oss;
			oss << PoisonsDiagnosed.at(poison) << " Poison" << endl;
			::gtk_label_set_text(diagnosisText, (const gchar *)( &( oss.str().at(0) ) ) );
		}
	}
	...
	for(unsigned clause = 0; clause < clauseVariableList.at(statementIndex).size(); clause++)
	{
		...
		if(localVariableValue == -1)
		{
			...
			if(askTheUser)
			{
				...
				ostringstream oss;
				oss << "Does the patient have the following symptom?"
					<< endl << "   " << localVariableName;
