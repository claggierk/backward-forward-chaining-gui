...
extern GtkLabel* goalText;
extern GtkLabel* diagnosisText;
extern GtkLabel* treatmentText;
extern GtkLabel* questionText;
extern bool gQuestionAnswered;
extern bool gYES;
...
void ForwardChaining::Trigger(string poison)
{
	...
	} else cout << endl << "Searching for treatments for " << poison << " poisoning." << endl;
	...
	{
		clauseVariableStatementIndex =
		       	ProcessStatementClauses(conclusionQueue.front().clauseVariableListIndex,
				       	conclusionQueue.front().variableName);
		if(clauseVariableStatementIndex == -1)
		{
			if(conclusionQueue.front().clauseVariableListIndex < clauseVariableList.size() - 1)
				conclusionQueue.front().clauseVariableListIndex = conclusionQueue.front().clauseVariableListIndex + 1;
			else conclusionQueue.pop();
			continue;
		}
...
//							cout << endl << " ----- INFO: Non-leaf conclusion located";
							fout << endl << " ----- INFO: Non-leaf conclusion located";
						}
					}
				}
				...
				for(unsigned int conclusion = 0; conclusion < conclusionList.at(clauseVariableStatementIndex).size(); conclusion++)
				{
					...
					treatments << conclusionList.at(clauseVariableStatementIndex).at(conclusion) << endl;
				}
				::gtk_label_set_text(treatmentText, (const gchar *)( &( treatments.str().at(0) ) ) );
				cout << endl << " ****************************************************";
				fout << endl << " ****************************************************";
//				break;
			}
...
//			{
	...
	cout << endl << " ****************************************************"
		<< endl << " ***** FORWARD CHAINING COMPLETED *******************"
		<< endl << " ****************************************************";
	::gtk_label_set_text(questionText, "Diagnosis and Treatment Complete");
}
	...
	for(unsigned variable = 0; variable < variableList.size(); variable++)
	{
		for(unsigned int currentConclusionList = 0; currentConclusionList < conclusionList.size(); currentConclusionList++)
		{
			...
			for(vector<string>::iterator iter = conclusionList.at(currentConclusionList).begin(); iter != conclusionList.at(currentConclusionList).end(); iter++)
			{
				...
				{
//					cout << endl << " ----- INFO: variable list and conclusion list have "
//						<< conclusionList.at(currentConclusionList).at(conclusion) << " in common."
//						<< endl << "             Removing from the conclusion list";

					fout << endl << " ----- INFO: variable list and conclusion list have "
						<< conclusionList.at(currentConclusionList).at(conclusion) << " in common."
						<< endl << "             Removing from the conclusion list";
	...
	if(ForwardChaining::sVerboseOutput)
	{
		...
		for(unsigned clause = 0; clause < clauseVariableList.size(); clause++)
		{
			...
			for(unsigned variable = 0; variable < clauseVariableList.at(clause).size(); variable++)
			{
				...
				clauseVariableListFile << "   " << clauseVariableList.at(clause).at(variable).clauseName << ": "
					<< clauseVariableList.at(clause).at(variable).clauseValue << endl;
			}
			cout << endl;
			fout << endl;
		...
		for(unsigned clause = 0; clause < clauseVariableList.at(statement).size(); clause++) {
			...
			if(statementClauseVariableWithoutNOT.at(0) == '!') {
				...
				for(unsigned character = 1; character < statementClauseVariableWithoutNOT.size(); character++) {
					tempStatementClauseVariableWithoutNOT = tempStatementClauseVariableWithoutNOT
										+ statementClauseVariableWithoutNOT.at(character);
				}
				statementClauseVariableWithoutNOT = tempStatementClauseVariableWithoutNOT;
			}
//fout << endl << "statement = " << statement+1 << endl;
//fout << endl << "clause = " << clause << endl;
//fout << endl << "statementClauseVariableWithoutNOT = " << statementClauseVariableWithoutNOT << endl;
...
//				cout << endl << " ----- INFO: Found: " << foundVariable << " at Statement: " << statement+1 << " --- Clause: " << clause;
				fout << endl << " ----- INFO: Found: " << foundVariable << " at Statement: " << statement+1 << " --- Clause: " << clause;
				foundStatementIndex = statement;
				break;
			}
		}
//		if(foundStatementIndex == -1) { continue; }  // go to the next statement if not found
		if(foundStatementIndex != -1) { break; }
	}

		if(foundStatementIndex == -1) { return foundStatementIndex; }
...
//fout << endl << "clause = " << clause << endl;
//fout << endl << "foundStatementIndex = " << foundStatementIndex << endl;
//fout << endl << "clauseVariableList size = " << clauseVariableList.at(foundStatementIndex).size() << endl;
...
//			cout << "\n ***** Deal with " << statementClauseVariableWithoutNOT;
			...
			for(unsigned nonLeafConclusion = 0; nonLeafConclusion < nonLeafConclusionList.size(); nonLeafConclusion++)
			{
				cout << "\n ***** statementClauseVariableWithoutNOT: " << statementClauseVariableWithoutNOT << " ??? "
					<< nonLeafConclusionList.at(nonLeafConclusion) << " = nonLeafConclusionList.at(nonLeafConclusion): ";
				fout << "\n ***** statementClauseVariableWithoutNOT: " << statementClauseVariableWithoutNOT << " ??? "
					<< nonLeafConclusionList.at(nonLeafConclusion) << " = nonLeafConclusionList.at(nonLeafConclusion): ";
...
//						cout << endl << " ----- INFO: Retreived data from variable list concerning: "
//							<< variableList.at(variable).variableName << ": " << variableList.at(variable).variableValue;
						fout << endl << " ----- INFO: Retreived data from variable list concerning: "
							<< variableList.at(variable).variableName << ": " << variableList.at(variable).variableValue;
			...
			if(foundPopulatedInVariableList) { continue; }
			...
			ostringstream oss;
			oss << statementClauseVariableWithoutNOT << "?";
			::gtk_label_set_text(questionText, (const gchar *)( &( oss.str().at(0) ) ) );
			string response;
			//cin >> response;
			gQuestionAnswered = false;
			while(!gQuestionAnswered)
			{
				// wait until user clicks YES or NO
			}
			gQuestionAnswered = false;
			...
			if     (response == "Yes") { updatedVariable = 1; }
			else if(response == "No")  { updatedVariable = 0; }
//			cout << endl << "\n You answered: \"" << response << "\"";
			...
			if(clauseVariableList.at(foundStatementIndex).at(clause).clauseName.at(0) == '!') {
				...
				if     (updateVal == 0) { updatedVariable = 1; }
				else if(updateVal == 1) { updatedVariable = 0; }
			}
